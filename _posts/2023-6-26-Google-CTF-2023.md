...
Không nghĩ nhiều, mình thử debug LUA bằng gdb rồi tìm flag trong memory, lúc đầu không nghĩ nó thành công như ai ngờ lại được <(").
...
Ta có thể inject shebang lên đầu file `script.sh` thông quan biến `name`. Thông thường shebang chỉ nhận duy nhất một tham số, để chạy được với nhiều tham số mình sử dụng `/usr/bin/env`.
...
```
Giờ khi vòng lặp bị break, đơn giải chương trình chỉ `push r12` 2 lần rồi gọi  `dprintf(v10,"Somehow you got here??\n","Somehow you got here??\n")`.
...
Lần này, ta không thể ghi đè ở bấy kì đâu trên binary, nên mình sẽ tập trung ghi đè ở libc.
...
Bài này hơi khó một chút ở cộng đoạn rev.
...
```
Oh, hàm `expand_string` sẽ check chuỗi `hmm` có định dạng `$...` không, nếu đúng định dàng thì sẽ gọi `s = getenv(hmm+1)` rồi để `src[0]=s` nếu s khác NULL.

Ở hàm `unpack_string` ta ý 2 đoạn code `idx = data_arr[i];` và `hmm += idx;` nên mình rút ra:
* `data_arr` lưu độ dài tên biến môi trường ta cần lấy.
...
Chúng ta đã lấy được giá trị của 3 biến môi trường, tuy nhiên flag đã bị che lạ ở hàm `censor_string`.
...
```
Hành vi của hàm `unpack_bools` không khác gì nhiều `unpack_string`, tuy nhiên ở hàm `fix_corrupt_booleans` ta thấy `v3 = &a1->handler_ptr[a1->cur_next];` bị bug `ouf-of-bond` nếu 
`a1->cur_next` < 0, chưa kể nếu `v3[i]` khác 0 thì nó sẽ ghi đè thành byte '\x01` thông qua `v3[i] = v3[i] != 0;`.

Hmm, ta có tận dụng bug để v3 để trỏ vào flag, từ đó ghi đè byte `C` thành byte `\x01` tù đó bypass hàm  `censor_string`.
